...
If you need this data to be merged with the holdout set, you can read it in inside the preprocessing function ("clean_df") in the submission script (submission.py/submission.R). Add the code needed for merging and preprocessing of the external data to this function. You should also add a .csv file with the external data to your Github repository (root folder). In case you use R, add 'COPY *.csv /app' to the 'r.Dockerfile' which is in your repository (in case it is missing currently).
...
::: {.callout-note appearance="simple" collapse="true" icon=false}
## What if I am using another version of Python or R? 
It's better to stick to the same version unless that's the only way to use a particular function or package (for example, the package is only supported in the older version of Python/R). 
