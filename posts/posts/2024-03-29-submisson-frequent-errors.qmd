...
- if you recode continuous variables into categorical, make sure that the categories are not too small; otherwise there is a risk that some category will be missing in the preprocessed holdout data. Remember that not all values of **continuous** variables that are in the holdout data are included in the fake data (fake data would be too large). 
- you've edited the function clean_df in submission.py/submission.r so that it can be applied to the holdout data. For example, inside this function you use the argument (df), not a custom variable name you used during preprocessing the training data
- you don't load the training data in the submission.py/submission.r. It shouldn't be loaded because the clean_df function will be applied to the holdout data which will be provided as the argument to this function
- you don't use the file with the outcome in the clean_df function; if you need to select only observations for which the outcome is not missing, you can use the variable 'outcome_available' (described [here](/posts/posts/2024-03-20-prefer-datasets.qmd){target="_blank"})
