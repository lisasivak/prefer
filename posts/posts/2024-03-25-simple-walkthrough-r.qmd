...
```{{r}}
library(data.table) # <--- to read data faster

# loading data (predictors)
train <- fread("path to the data which is NOT in your local repository/PreFer_train_data.csv") 

# loading the outcome
outcome <- fread("path to the data which is NOT in your local repository/PreFer_train_outcome.csv") 
```
...
clean_df <- function(df, background_df = NULL){
    ...
    df <- df[ , ..keepcols ] #<------ added `..` to select these columns from the data.table
...
```{{r}}
# load the fake data
fake <- fread("PreFer_fake_data.csv")
...
8. "packages.R": you don't have to edit this file now, because you only used "data.table", and it is already in this file  
