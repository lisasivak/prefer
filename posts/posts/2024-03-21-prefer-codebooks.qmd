---
title: "A guide on PreFer codebooks"
description: |
  Here is some useful information that can help navigate the PreFer codebooks   
categories:
  - LISS
  - dataset
  - guide
...
This codebook contains metadata on all 31665 XXX CHECK THIS variables in the dataset `PreFer_train_data.csv` and the 33 variables XXX CHECK THIS in the `PreFer_background_data.csv`. 
...
You can also do that via R or Python. R:
```{r message = FALSE}
...
```
Let's imagine that in our analyses we only want to select variables that are categorical, numeric, or date or time. Note that we want to select these variable in the Core survey data (the train data) and do not want to include variables from the Background data (see "Explanation of datasets"-blogs HAVE TO UPDATE THIS for further information)
```{r}
...
The Python equivalent for the above steps would be something like:
```
codebook <- read.csv("PreFer_codebook.csv")
codebook[ codebook$var_name == "cp12e007", ]
select_type <- c("categorical", "numeric", "date or time")
list_variables <- codebook[ (codebook$type_var %in% select_type) & 
                              codebook$dataset == "PreFer_train_data.csv", ]$var_name
fake_data <- read.csv("PreFer_fake_data.csv")
fake_data_sel <- fake_data[ , list_variables ]
```
