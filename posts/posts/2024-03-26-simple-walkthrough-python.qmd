...
Let's imagine that you want to add one predictor to the model that is already in the repository: highest education level (variable "oplmet_2020", as you found using the codebooks). To produce this model you should use the template functions that are already in the repository:  "clean_df" for preprocessing the data from the "submission.py", "train_save_model" from "training.py" and "predict_outcomes" from "submission.py".   

Overall steps: preprocessing the data --> training the model --> testing on fake data if the preprocessing function and prediction model works on fake data --> editing "submission.py" and "training.py" to prepare a submission.   

1. Create new script in the environment for python that you normally use, save the file in the folder with your cloned repository. Import necessary packages (in this case we only need pandas, scikit learn, and joblib to save the model) and load training data and outcome. !VERY IMPORTANT: do not add the data to the folder on your computer with all the files from your repository. If you put the data in this folder and then upload all the changes online, the data will achanges online, the data will be made public in your . 
...
2. Create a blank script in the environment for python that you normally use, save it into the folder with all repository files. Copypaste into this script the `clean_df` function from the "submission.py" and `train_save_model` function from the "submission.py".  

3. Edit the `clean_df` function - add the new variable:  
...
def clean_df(df, background_df=None):
    ...
    df["oplmet_2020"] = df["oplmet_2020"].fillna(df["oplmet_2020"].mode()) # <-----ADDED VARIABLE

    # Selecting variables for modelling
    keepcols = [
        "nomem_encr",  # ID variable required for predictions,
        "age"          # newly created variable
        "oplmet_2020"  # <--------ADDED VARIABLE
    ] 
    ...
    return df
```
4. Edit the `train_save_model` function: add the new variable
...
def train_save_model(cleaned_df, outcome_df):
    ...
    # Fit the model
    model.fit(model_df[['age', 'oplmet_2020']], model_df['new_child']) # <-------- ADDED VARIABLE
...
2.
First, you need to preprocess the data. Create a new script in the environment that you normally use for python, save it in the folder with your cloned repository. This is the script that you should update to produce a model.    
...
10. push the changes ("Push origin") (i.e. update your online repository)  
