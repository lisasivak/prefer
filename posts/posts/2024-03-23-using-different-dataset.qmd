...
Here we describe how you can (maybe) fruitfully combine the different datasets `PreFer_train_data.csv`, `PreFer_train_background_data.csv`, and `PreFer_train_supplementary_data.csv`. ([here](UPDATETHIS) you can find a guide on these different datasets)
...
The most important dataset of the PreFer data challenge is the `PreFer_train_data.csv` dataset ([click here](UPDATE THIS) for information on the datasets that are available). This contains data on all LISS respondents who were between the ages of 18 and 45 in 2020 (i.e., had birthyears between 1975 and 2002). [Click here](UPDATETHIS) for an explanation on how to quickly read-in the data 
...
```{r eval = FALSE}
background <- data.table::fread("PreFer_train_background_data.csv", 
...
```{r eval = FALSE}
supplement <- data.table::fread("PreFer_train_supplementary_data.csv", 
                                keepLeadingZeros = TRUE, 
                                data.table = FALSE) 
```
...
To make this work
1) don't forget to add `"tidyverse"` to `packages.R`
