...
You can now make use of the variable `mean_income_imp` in your `train_save_model`-file. To make this work don't forget to add `"tidyverse"` to `packages.R`! [Here](https://github.com/gertstulp/fertility-prediction-challenge/tree/background) you can see that it passes the check.
...
And then use your usual method to train on the data. This will work because your method will make use of these new cases, but the structure of the dataset has not changed (no new variables were added). If you would create a novel variable on the basis of the supplementary data and use this variable in your model, it would not work on the holdout data, because that variable will not exist in the holdout data. [Here](https://github.com/gertstulp/fertility-prediction-challenge/blob/supplementarydata) you can see that it passes the check.