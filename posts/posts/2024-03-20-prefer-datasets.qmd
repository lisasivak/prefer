...
Here we describe the different datasets that are available in the PreFer data challenge that are all based on the [LISS panel](https://www.lissdata.nl/). There are five datasets available to PreFer participants: 1) `PreFer_train_data.csv`, 2) `PreFer_train_outcome`, 3) `PreFer_train_background_data.csv`, 4) `PreFer_train_supplementary_data.csv`, and 5) `PreFer_train_supplementary_outcome.csv`. The three datasets `PreFer_holdout_data.csv`, `PreFer_holdout_outcome`, and `PreFer_holdout_background_data.csv` will only be available to PreFer organisers. There are also three datasets of fake data, available for participants, that are mostly in place for testing whether your submission will be able to run on the holdout data (`PreFer_fake_data.csv`, `PreFer_fake_outcome`, and `PreFer_fake_background_data.csv`). This all sounds overwhelming, so let's quickly get to explaining them. 

To navigate these datasets and get a better understanding of the many variables, there are also machine-readable (and human readable) codebooks available: [click here](2024-03-21-prefer-codebooks.qmd) for a guide on how to use the codebooks.
...
[The codebooks]((2024-03-21-prefer-codebooks.qmd)) are particularly useful in navigating the merged Core survey dataset. The datasets `PreFer_train_data.csv`, `PreFer_train_supplement.csv`, and `PreFer_holdout_data.csv` all come from this merged Core survey dataset. 
...
1. `nomem_encr`: this is the unique identifier and can be used to link the train data to the other datasets (`PreFer_train_background.csv`, `PreFer_train_supplement.csv`, and `PreFer_train_outcome`). Note that this identifier is a pseudonym of the original `nomem_encr` identifier in the LISS data. 
...
This dataset contains the same variables as `PreFer_train_background_data.csv`, but it includes fake data with randomly dawn values from each variable in the background data. This dataset is used for testing whether the developed preprocessing scripts and trained models work when the original analyses included `PreFer_train_background_data.csv`. A failed test using this data likely means a failed submission on the holdout data.  
...
This is a fake dataset that is used for testing whether the method submitted will run on the holdout data. 
...
*** In the folder "training_data" ***

1. PreFer_train_data.csv

This is the main training data for data challenge participants. It includes respondents born between 1975 and 2002.

2. PreFer_train_outcome.csv

This is the outcome [who had a(nother) child in 2021-2023] for respondents in the training data (PreFer_train_data.csv).


*** In the folder "other_data" ***

3. PreFer_train_background_data.csv

This is the dataset with background variables that can be used to augment the PreFer_train_data.csv.

4. PreFer_train_supplementary_data.csv

This is the dataset with information on respondents that are *not* included in PreFer_train_data.csv, but that can be used to augment the PreFer_train_data.csv. It includes respondents born prior to 1975 and after 2002.

5. PreFer_train_supplementary_outcome.csv

This is the outcome [who had a(nother) child in 2021-2023] for respondents in the supplementary data (PreFer_train_data.csv).

6. PreFer_fake_data.csv

This is a fake dataset that is used for testing whether the method submitted will run on the holdout data. 

7. PreFer_fake_background_data.csv

This is a fake dataset that is used for testing whether the method submitted will run on the holdout data. 

8. PreFer_fake_outcome.csv

This is a fake dataset that is used for testing whether the method submitted will run on the holdout data. 


*** In the folder "codebooks" ***

9. PreFer_codebook.csv

This provides a machine-readable codebook for *all* variables in the dataset(s). 

10. PreFer_codebook_summary.csv

This provides a machine-readable summary codebook that is useful for tracking the consistency of the measurement of variables across waves. 
